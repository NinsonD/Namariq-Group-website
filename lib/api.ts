...
// Specialized Materials API
export const specializedMaterialsApi = {
  async getAll(params?: {
    category?: string
    search?: string
    page?: number
    ...
    if (process.env.NODE_ENV === 'development') {
      let filteredMaterials = mockSpecializedMaterials

      if (params?.category) {
        filteredMaterials = filteredMaterials.filter(m => m.category === params.category)
      }
      ...
      if (params?.search) {
        ...
        filteredMaterials = filteredMaterials.filter(m =>
          ...
          m.shortDesc.toLowerCase().includes(searchTerm)
        )
      }
    ...
    const response = await fetch(`${API_BASE_URL}/specialized-materials?${queryParams}`)
    return response.json()
  },

  async getBySlug(slug: string): Promise<ApiResponse<any>> {
    if (process.env.NODE_ENV === 'development') {
      const material = mockSpecializedMaterials.find(m => m.slug === slug)
      return {
        data: material,
        success: !!material,
    ...
    const response = await fetch(`${API_BASE_URL}/specialized-materials/${slug}`)
    return response.json()
  },

  async getCategories(): Promise<ApiResponse<string[]>> {
    if (process.env.NODE_ENV === 'development') {
      const categories = [...new Set(mockSpecializedMaterials.map(m => m.category))]
      return {
        data: categories,
        success: true
      }
    ...
    const response = await fetch(`${API_BASE_URL}/specialized-materials/categories`)
    return response.json()
  }
}
